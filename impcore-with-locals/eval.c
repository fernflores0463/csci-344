...
/* make [[f]] the function denoted by [[e->u.apply.name]], or call [[runerror]] 48c */
            ...
            case USERDEF:
                /* apply [[f.u.userdef]] and return the result 49b */
                {
                    ...
                    Namelist  lvs = f.u.userdef.locals;
                    Valuelist vs = evallist(e->u.apply.actuals, globals,
                                                            functions, formals);

                    Valenv v = mkValenv(xs, vs);
                    while(lvs != NULL ){
                        /* code */
                        bindval(lvs->hd, 0, v);
                        lvs = lvs->tl;
                    ...
                    return eval(f.u.userdef.body, globals, functions, v);
                }
            case PRIMITIVE:
          ...
          /* apply \impcore\ primitive [[println]] to [[vs]] and return 1040d */
                        {
                            checkargc(e, 1, lengthVL(vs));
                            Value v = nthVL(vs, 0);
                            print("%v\n", v);
           ...
           /* apply \impcore\ primitive [[printu]] to [[vs]] and return 1041a */
                        {
                            checkargc(e, 1, lengthVL(vs));
                            Value v = nthVL(vs, 0);
                            print_utf8(v);
