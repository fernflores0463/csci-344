...
Exp reduce_to_exp(int code, struct Component *components) {
    ...
    /* cases for \impcore's [[reduce_to_exp]] added in exercises 943d */
    /* add your syntactic extensions here */
    /* cases for \impcore's [[reduce_to_exp]] added in exercises 958b */
...
/* arrays of shift functions added to \impcore\ in exercises 952c */
/* for each new row added to exptable, add an array of shift functions here */
/* arrays of shift functions added to \impcore\ in exercises 959a */
...
struct ParserRow exptable[] = {
  ...
  /* rows added to \impcore's [[exptable]] in exercises 952d */
  /* add a row here for each new syntactic form of Exp */
  /* rows added to \impcore's [[exptable]] in exercises 959b */
...
struct Usage usage_table[] = {
    ...
    { BEGIN,  "(begin exp ... exp)" },
    /* \impcore\ [[usage_table]] entries added in exercises 959c */
    { SUGAR(CAND), "(&& exp exp)" },
    { SUGAR(COR),  "(|| exp exp)" },
...
void check_def_duplicates(Sourceloc source, Def d) {
    if (d->alt == DEFINE && duplicatename(d->u.define.userfun.formals) != NULL && duplicatename(d->u.define.userfun.locals) != NULL){
        synerror(source,
