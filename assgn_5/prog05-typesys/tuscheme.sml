...
fun tysubst (tau, varenv) =
  ...
  (* definition of [[renameForallAvoiding]] for {\tuscheme} ((prototype)) 465 *)
      ...
      let val avoid = union (alphas, union (captured, freetyvars tau))
          val (betas, _) = foldr (fn (alpha, (betas, avoid)) =>
                            let val beta = if member alpha captured then freshName(alpha, avoid)
                                           else alpha
                            in (beta :: betas, insert(beta, avoid))
                            end)
                        ([], avoid)
                        alphas
          val renameEnv = bindList(alphas, map TYVAR betas, emptyEnv)
      in FORALL(betas, tysubst(tau, renameEnv))
      end
      (* type declarations for consistency checking *)
...
fun typeof (e : exp, Delta : kind env, Gamma : tyex env) : tyex =
  let fun ty (LITERAL (NUM n)) = inttype
        ...
        | ty (LITERAL (PAIR (h, t))) =
          (case t of NIL => listtype (ty (LITERAL h)) |
           _ => let val t_h = ty (LITERAL h)
                val t_t = ty (LITERAL t)
                val t_l = listtype t_h
            in if eqType(t_l, t_t) then t_t
            else raise TypeError ("List of different types of elements")
            end)
                 ...
                 in
                        if(eqType(tau_e1, booltype)) then
                                if(eqType(tau_e2, tau_e3)) then
                                        tau_e2
                                else raise TypeError "e2 and e3 not same type"
                        else raise TypeError "e1 (conditional) not a boolean"  
                end )
        | ty (WHILEX (e1, e2)) = 
                (let val tau_e1 = ty e1
                 ...
                 in
                   if (eqType (tau_e1, booltype)) then
                     unittype
                   else raise TypeError "Conditional is not type bool"
                 end)
        | ty (BEGIN es) = (case es of
		[] => unittype
		| [e] => ty e
		| e::es => (ty e; ty(BEGIN es)))
        | ty (LETX (LET, bs, body)) = 
		let val new_te =
			bindList (map (fn (x, e) => x) bs, map (fn (x, e) => typeof(e, Delta, Gamma)) bs, Gamma)
		in typeof (body, Delta, new_te)
		end
        | ty (LETX (LETSTAR, bs, body)) = 
        (case bs of
                nil => typeof( body, Delta, Gamma)
              | b::bs => ty (LETX (LET, [b], LETX (LETSTAR, bs, body))))
        | ty (LETRECX (bs, body)) = 
          let val new_gamma = foldl (fn (((x, y), exp), new_gamma) => 
           case exp of LAMBDA _ => bind (x, asType (y, Delta), new_gamma)
           | _ => raise TypeError ("Not a Lambda")) Gamma bs
           val stuff = (fn ((x, y), e) => if  eqType(typeof(e, Delta, new_gamma), asType(y, Delta))
                                      then ()
                                      else raise TypeError ("Type mismatch"))
           val _ = List.app stuff bs
          in typeof(body, Delta, new_gamma)
          end
        | ty (LAMBDA (formals, body)) = (
		let fun recTyp ((var, typ)::tail, tenv) = (*could be done by using map as well*)
			...
			| asTypeRec (nil, kindeniv) = Delta
		    
                    val args = recTypList(formals)
		    val kinding = asTypeRec(formals, Delta)
		    val exptype = typeof(body, Delta, recTyp(formals, Gamma))
		in (FUNTY (args, exptype))
		end)
        | ty (APPLY (f, actuals)) =
             let val actualtypes = map ty actuals
                 val funty = ty f

             in  case funty of FUNTY(args, result) => 
             if eqTypes (actualtypes, args) then
                   result
                 else
                   raise TypeError ("Function arguments do match their types")
                   | _ => raise TypeError ("Not a function :'( ")
             end
        | ty (TYLAMBDA (alphas, e)) = 
        let val types = map(fn _ => TYPE) alphas
            val d = bindList(alphas, types, Delta)
            val tau = typeof(e, d, Gamma)
            val n = inter (alphas, freetyvarsGamma Gamma)
            in
            case n of [] => FORALL(alphas, tau)
            | alpha :: _ => raise TypeError("The varianle appears in the type environment")
            end
...
fun elabdef (d : def, Delta : kind env, Gamma : tyex env) : tyex env * string =
  case d
    ...
    in (bind(name, tau_e, Gamma), typeString(tau_e))
    end
     | EXP e => elabdef (VAL ("it", e), Delta, Gamma)
     | DEFINE (name, tau, lambda as (formals, body)) =>
     let val fun_type = FUNTY (map (fn (n, ty) => ty) formals, tau)
      in  elabdef(VALREC(name, fun_type, LAMBDA lambda), Delta, Gamma)
      end
     | VALREC (name, tau, e) =>
     let val g = bind(name, tau, Gamma)
     val tau = asType (tau, Delta)
     val t = typeof(e, Delta, g)

     in case e of
                   LAMBDA _ => if not (eqType( t , tau)) then
        raise TypeError "var is declared to have a different than the specified type"
      else (g, typeString tau)
                 | _ => raise TypeError ("not a lambda")
